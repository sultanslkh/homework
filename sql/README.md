...
SELECT t.id, t.name,(select tf.name from Company tf where tf.name='air_france') from Passenger t
...
SELECT t.passenger from Pass_in_trip t
SELECT t.passenger, t.trip FROM Pass_in_trip t ORDER BY t.passenger DESC
...
SELECT \* from Passenger t where t.name LIKE 'A%'
SELECT t.passenger FROM Pass_in_trip t where t.passenger > 1 ORDER BY t.passenger DESC
